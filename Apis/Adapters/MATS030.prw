...
WSRESTFUL customerVendor DESCRIPTION "Cadastro de Cliente/Fornecedor" //"Cadastro de Cliente/Fornecedor"
    ...
    WSSYNTAX "/customerVendor/{Order, Page, PageSize, Fields}" ;
    PATH "/customerVendor"	

    WSMETHOD POST Main ;
    DESCRIPTION "Cadastra um Clientes/Fornecedores" ;
    WSSYNTAX "/customerVendor/{Fields}" ;
    PATH "/customerVendor"	

    WSMETHOD GET Type ;
    DESCRIPTION "Retorna todos Clientes ou Fornecedores" ;
    WSSYNTAX "/customerVendor/{Type}{Order, Page, PageSize, Fields}" ;
    PATH "/customerVendor/{Type}"	

    WSMETHOD GET EspecID ;
    DESCRIPTION "Retorna um cliente/fornecedor específico" ;
    WSSYNTAX "/customerVendor/{Type}/{customerVendorId}{Order, Page, PageSize, Fields}" ;
    PATH "/customerVendor/{Type}/{customerVendorId}"		
...
WSMETHOD GET Main WSRECEIVE Order, Page, PageSize, Fields WSSERVICE customerVendor
	...
	oApiManager := APIMANAGER():New("MATS030","2.006") 
	...
	If !lRetForn .And. !lRetCli
		lRet := .F.
		oApiManager:SetJsonError("404","Erro ao acessar o cliente/fornecedor!", "Cliente/Fornecedor não encontrado.",/*cHelpUrl*/,/*aDetails*/)
	EndIf
...
WSMETHOD POST Main WSRECEIVE Order, Page, PageSize, Fields WSSERVICE customerVendor
	...
	Local oApiManager 	:= APIMANAGER():New("MATS030","2.006")
...
WSMETHOD GET EspecID PATHPARAM Type, customerVendorId WSRECEIVE Order, Page, PageSize, Fields WSSERVICE customerVendor
	...
	If Len(AllTrim(Self:customerVendorId)) >= nLenFields
		If Self:Type == "1"
...
WSMETHOD PUT EspecID PATHPARAM Type, customerVendorId WSRECEIVE Order, Page, PageSize, Fields WSSERVICE customerVendor
	...
	If lRet
		If Self:Type == "1"
			If Len(AllTrim(Self:customerVendorId)) >= nLenFields
				If SA1->(Dbseek(xFilial("SA1") + Self:customerVendorId))
					lRet := ManutCliente(oApiManager, Self:aQueryString, 4, aJson, Self:customerVendorId, oJson, cBody)
				Else
					oApiManager:SetJsonError("404","Erro ao alterar o cliente!", "Cliente não encontrado.",/*cHelpUrl*/,/*aDetails*/)
				EndIf
			Else
				lRet := .F.
				oApiManager:SetJsonError("400","Erro ao alterar o cliente!", "O CustomerVendor ID deve possuir pelo menos "+ cValToChar(nLenFields) +" caracteres.",/*cHelpUrl*/,/*aDetails*/)
		...
		ElseIf Self:Type == "2"
			If Len(AllTrim(Self:customerVendorId)) >= nLenFields
				If SA2->(Dbseek(xFilial("SA2") + Self:customerVendorId))
					lRet := MATS020(oApiManager, Self:aQueryString, 4, Self:customerVendorId, oJson, cBody)
				Else
					oApiManager:SetJsonError("404","Erro ao alterar o Fornecedor!", "Fornecedor não encontrado.",/*cHelpUrl*/,/*aDetails*/)
				EndIf
			Else
				lRet := .F.
...
WSMETHOD DELETE EspecID PATHPARAM Type, customerVendorId WSRECEIVE Order, Page, PageSize, Fields WSSERVICE customerVendor
	...
	If Self:Type == "1"
		If Len(AllTrim(Self:customerVendorId)) >= nLenFields
			...
			Else
				oApiManager:SetJsonError("404","Erro ao excluir o cliente!", "Cliente não encontrado.",/*cHelpUrl*/,/*aDetails*/)
			EndIf
		Else
			lRet := .F.
	...
	ElseIf Self:Type == "2"
		If Len(AllTrim(Self:customerVendorId)) >= nLenFields
			...
			Else
				lRet := .F.
				oApiManager:SetJsonError("404","Erro ao excluir o Fornecedor!", "Fornecedor não encontrado.",/*cHelpUrl*/,/*aDetails*/)
			EndIf
		Else
			lRet := .F.
...
Static Function MontaCab(oJson, aCab)
	...
	If AttIsMemberOf(oJson, "name") .And. AttIsMemberOf(oJson, "id") .And. !Empty(oJson:id) .And. !Empty(oJson:name)
		If AllTrim(oJson:name) $ "INSCRICAO ESTADUAL"
			aAdd( aCab, {'A1_INSCR'		,oJson:id, Nil}) 
		ElseIf AllTrim(oJson:name) $ "INSCRICAO MUNICIPAL"
			aAdd( aCab, {'A1_INSCRM' 	,oJson:id, Nil}) 
